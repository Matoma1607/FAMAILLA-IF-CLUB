...
const AsistenciaView = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {isNewSocioModalOpen && (
        <div 
          className="fixed inset-0 z-[110] overflow-y-auto bg-slate-900/60 backdrop-blur-sm px-4 py-6 sm:px-6"
          onClick={() => setIsNewSocioModalOpen(false)}
        >
          <div className="flex min-h-full items-center justify-center">
            <div 
              className="bg-white rounded-[2rem] sm:rounded-[2.5rem] w-full max-w-md shadow-2xl animate-fade-in relative flex flex-col"
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input required className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none focus:border-primary transition-all text-sm" value={newSocio.nombre || ''} onChange={e => setNewSocio({...newSocio, nombre: e.target.value.toUpperCase()})} />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <input required className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none focus:border-primary transition-all text-sm" value={newSocio.nombreTutor || ''} onChange={e => setNewSocio({...newSocio, nombreTutor: e.target.value.toUpperCase()})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-1">WhatsApp</label>
                  <input required className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl font-bold outline-none focus:border-primary transition-all text-sm" placeholder="381..." value={newSocio.telefonoTutor || ''} onChange={e => setNewSocio({...newSocio, telefonoTutor: e.target.value})} />
                </div>
                <button type="submit" disabled={creatingSocio} className="w-full bg-emerald-500 text-white py-4 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-500/20 mt-2 flex items-center justify-center space-x-2 active:scale-[0.98] transition-all">
                  {creatingSocio ? <Loader2 className="animate-spin" size={18} /> : <span>REGISTRAR Y PRESENTAR</span>}
                </button>
              </form>
            </div>
          </div>
